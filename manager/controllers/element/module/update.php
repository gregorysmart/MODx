...
$groupsarray = array();
$moduleugs = $modx->getCollection('modModuleUserGroup',array('module' => $module->get('id')));

if ($moduleugs != NULL) {
    $moduleugs_count = count($moduleugs);
    foreach ($moduleugs as $ug) {
        $groupsarray[$i] = $ug->get('usergroup');
    }
}
$usergroups = $modx->getCollection('modUserGroup');
foreach ($usergroups as $ug) {
    if (in_array($ug->get('id'),$groupsarray)) {
        $ug->set('checked',true);
...
$onModFormRender = $modx->invokeEvent('OnModFormRender',array('id' => $module->get('id')));
