...
if ($old_module == null) return $modx->error->failure($modx->lexicon('module_err_not_found'));
...
if ($module->save() === false) {
	return $modx->error->failure($modx->lexicon('module_err_duplicate'));
}
...
foreach ($old_module->dependencies as $old_dep) {
	...
	if ($new_dep->save() == false) {
		return $modx->error->failure($modx->lexicon('module_err_duplicate_dependancy'));
    }
}
...
foreach ($old_module->usergroups as $old_ug) {
	...
	if ($new_ug->save() == false) {
		return $modx->error->failure($modx->lexicon('module_err_duplicate_usergroup'));
    }
}
