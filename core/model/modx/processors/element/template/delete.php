...
foreach ($template->ttvs as $ttv) {
	if (!$ttv->remove()) $modx->error->failure($modx->lexicon('tvt_err_remove'));
}
...
if (count($resources) > 0) {
	...
	$modx->error->failure($modx->lexicon('template_err_in_use').$ds);
}

/* make sure isn't default template */
if ($template->get('id') == $default_template) {
	$modx->error->failure($modx->lexicon('template_err_default_template'));
}
...
if ($template->remove() == false) {
	$modx->error->failure($modx->lexicon('template_err_delete'));
}
